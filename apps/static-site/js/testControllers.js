...
==============================================
  ...
  - Tests REST endpoints (getMany, getOne, create, update, deleteOne)
    for employees, positions, locations, and shifts controllers.
  - Uses fetch to send requests to the backend and records results.
  - Writes a Markdown summary file (testControllersResults/testControllersResults.md)
    with pass/fail status and links to detailed response/error files for each test.
...
const resultsFile = path
  ? path.join(resultsDir, "testControllersResults.md")
  : "./testControllersResults/testControllersResults.md";
...
async function testController(controller) {
  ...
  const dummy = {
    ...
    shifts: {
      id: 99999, // Use a high number to avoid PK conflict; required
  ...
  function record(endpoint, ok, error, details, payload = null) {
    ...
    if (fs && details) {
      ...
      fs.writeFileSync(
        filePath,
        `# ${controller} ${endpoint} Result\n\n${backLink}${requestSection}${responseSection}`
      );
      fileLink = `([details](${fileName}))`;
    }
    results[controller][endpoint] = ok
      ? `PASS ${fileLink}`
      : `FAIL - ${errSummary} ${fileLink}`;
    total++;
    if (ok) passed++;
    else failed++;
  }
